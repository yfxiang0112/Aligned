...
for file_idx, log_file in enumerate(log_files, 1):
    ...
    ' eval MSE '
    Y_true_con = load_npz(
        f'dataset/human/{data_name}_Y_con.npz').toarray()[test_idx]
...
print(
    f'f1 on test: {.5*(max(data_f1_lst)+min(data_f1_lst)): .4f} ± {.5*(max(data_f1_lst)-min(data_f1_lst)): .4f}')
print(
    f'f1 on KB: {.5*(max(kb_f1_lst)+min(kb_f1_lst)): .4f} ± {.5*(max(kb_f1_lst)-min(kb_f1_lst)): .4f}')
print(
    f'integrated F1: {.5*(max(bal_f1_lst)+min(bal_f1_lst)): .4f} ± {.5*(max(bal_f1_lst)-min(bal_f1_lst)): .4f}')
