...
def optimize_X(Y, X0, Omega, C, k, t, init_lr=1e-3, max_iter=1000, tol=1e-3, decay_rate=0.995, device=torch.device('cpu'), verbose=False):
    ...
    for epoch in range(max_iter):
        ...
        Xk_ = torch.clamp(torch.matrix_power(
            torch.round(exp_soft(X, t)), k), 0, 1)
        ...
        print(
            f'rounded |X_k-Y|_0 = {loss_round}, f1 = {f1: .6f}, approx slack: {torch.count_nonzero(Xk_ - torch.round(exp_power(X, k, t)))}')
        print(
            f'rounded before pow |X_k-Y|_0 = {loss_round_}, f1 = {f1_: .6f}\n')
...
if __name__ == "__main__":
    ...
    idx_map = [idx for idx, v in enumerate(
        gene_idx['precise1k_idx']) if v != -1]
    connectv_kb = connectv_kb[idx_map][:, idx_map]
    connectv_sup = connectv_sup[idx_map][:, [
        i for i in gene_idx['precise1k_idx'] if i != -1]]
    ...
    print(
        f'check X0^k == 1-exp(X0^k): {torch.count_nonzero(torch.clamp(torch.matrix_power(X0, k), 0, 1) - torch.round(exp_power(X0, k, t)))}')
    print(
        f'check X0^k == tanh(X0^k): {torch.count_nonzero(torch.clamp(torch.matrix_power(X0, k), 0, 1) - torch.round(tanh_power(X0, k)))}')

    # print(f'init loss = {torch.norm((torch.clip(torch.matrix_power(X0, k),0,1)-Y)[Omega],p="fro") ** 2}')
    print(
        f'init Xk-Y diff = {torch.count_nonzero((torch.clip(torch.matrix_power(X0, k), 0, 1)-Y)[Omega])}')
    print(
        f'init f1 = {f1_score(torch.clip(torch.matrix_power(X0, k), 0, 1)[Omega].flatten().detach().cpu().numpy(), Y[Omega].flatten().detach().cpu().numpy())}')

    #######################################

    # Solve the optimization problem
    X_opt, losses = optimize_X(Y, X0, Omega, C, k, t, init_lr=1e-3,
                               decay_rate=.995, max_iter=1000, device=device, verbose=True)
    ...
    print(f"Y = {Y}")
    print(
        f"Frobenius norm of X^k - Y: {torch.norm((X_opt_k - Y)[Omega], p='fro').item()}")
    print(f"L1 norm of X - X0:, {torch.norm(X_opt - X0, p=1).item()}")

    print(f'X_k rounded = \n{torch.round(X_opt_k)}')
    print(
        f'rounded X_k-Y diff = {torch.count_nonzero((torch.round(X_opt_k)-Y)[Omega])}')

    X_opt_round = torch.round(exp_soft(X_opt, t))
    print(f'X rounded = \n{X_opt_round}')
    print(f'|X-X0|_1 = {torch.norm(X_opt_round-X0, p=1)}')

    save_npz('scripts/klg_refine/X_opt.npz',
             coo_matrix(exp_soft(X_opt, t).detach().cpu().numpy()))
    save_npz('scripts/klg_refine/X_opt_k.npz',
             coo_matrix(X_opt_k.detach().cpu().numpy()))
