...
Y_deduction = KB.deduce(torch.tensor(X_train).to('cuda'))
...
kb_con_idx = (np.nonzero(np.sum((Y_deduction != 0) & (Y_deduction == Y_train), axis=0) / total > .1)[0].tolist())
...
go_annot_num = np.array([len(gene2go[g]) if g in gene2go.index else 0 for g in df_genes['gene_name']], dtype=np.float32)
