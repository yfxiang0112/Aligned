...
incons_dict_path = 'results/fig1_incons/incons_edges.json'
if not os.path.exists(incons_dict_path):
    incons_dict= {}
    for data_name, kb_name in combs:
        print(f'processing {data_name} data + {kb_name} kb')

        if os.path.exists(f'results/fig1_incons/data/{data_name}_Corr_P.npz')\
            and os.path.exists(f'results/fig1_incons/data/{data_name}_Corr_N.npz'):
            corr_P = torch.tensor(load_npz(f'results/fig1_incons/data/{data_name}_Corr_P.npz').toarray()).to(device)
            corr_N = torch.tensor(load_npz(f'results/fig1_incons/data/{data_name}_Corr_N.npz').toarray()).to(device)
        else:
            ...
            save_npz(f'results/fig1_incons/data/{data_name}_Corr_P.npz', coo_matrix(corr_P.cpu().numpy()))
            save_npz(f'results/fig1_incons/data/{data_name}_Corr_N.npz', coo_matrix(corr_N.cpu().numpy()))

        
        KB = RegulatoryKB(pos_trn_pth=f'results/fig1_incons/data/{data_name}_{kb_name}_KB_P.npz',\
                neg_trn_pth=f'results/fig1_incons/data/{data_name}_{kb_name}_KB_N.npz'\
...
for k, v in incons_dict.items():
    ...
    #plt.savefig(f'results/fig1_incons/piechart_{data_name}_{kb_name}.pgf', dpi=600, format='pgf')
    #plt.savefig(f'results/fig1_incons/piechart_{data_name}_{kb_name}.png', dpi=600)
    plt.show()
