...
# Read the log file
with open("scripts/klg_refine/log_exp_lr_decay.txt", "r") as f:
    log_data = f.read()
...
pattern = re.compile(
    ...
    r"rounded \|X_k-Y\|_0 =\s*([\d.]+), f1 =\s*([\d.]+)\s*"
)
...
for match in matches:
    ...
    loss1.append(np.sqrt(float(match[2])))
    loss2.append(float(match[3]))
    acc.append(1- int(match[4])/(4639**2))
    f1.append(float(match[5]))

iterations = iterations[:20]
total_loss, loss1, loss2, acc = total_loss[:20], loss1[:20], loss2[:20], acc[:20]
...
ax2.plot(iterations, f1, label="Dataset F1", color='tab:red', marker='s', linestyle=':')
...
ax3.plot(iterations, loss1, label="MSE loss: |Xk-Y|_F", color='tab:orange', marker='x', linestyle='--')
...
plt.title("Training Losses with Independent Y-Axes")
